...
###for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    ###frame = cv2.cvtColor(np.asarray(frame), cv2.COLOR_BGR2GRAY)    ###convert to gray
...
#    brown=cv2.inRange(hsv,brownMin,brownMax)  
    ...
    params.minArea = 1000 #r=9px           # min 1000 pixels squared
...
#    cv2.line(im_with_keypoints, (0,120), (319,120), (255,0,0), 1)
    cv2.line(im_with_keypoints, (319,0), (319,578), (255,0,0), 1)
    cv2.line(im_with_keypoints, (0,239), (638,239), (255,0,0), 1)
    ...
    for i in keypoints:   
        ...
        if len(centre) > 2:                 
            ...
            im_with_keypoints=cv2.circle(im_with_keypoints, (int(Xc), int(Yc)),5, (255, 255, 0), 2)
            cv2.putText(im_with_keypoints, "first blob", (int(x1 + 10), int(y1)), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255),1)
            cv2.putText(im_with_keypoints, "second blob", (int(x2 + 10), int(y2)), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255),1)
            cv2.putText(im_with_keypoints, "third blob", (int(x3 + 10), int(y3)), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255),1)
            
            cv2.putText(im_with_keypoints, "t1", (int((x2+x1)/2), int((y2+y1)/2)), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 0, 0),1)
            cv2.putText(im_with_keypoints, "t2", (int((x3+x1)/2), int((y3+y1)/2)), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 0, 0),1)
            cv2.putText(im_with_keypoints, "t3", (int((x2+x3)/2), int((y2+y3)/2)), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 0, 0),1)
     
            cv2.line(im_with_keypoints, (319,239), (int(Xc),int(Yc)), (0,255,0), 1)
            ...
            #print("distance t1=x2,x1: ",t1)
            t2 = math.sqrt((abs(x3 - x1)) ** 2 + (abs(y3 - y1)) ** 2)
            #print("distance t2=x3,x1: ",t2)
            t3 = math.sqrt((abs(x3 - x2)) ** 2 + (abs(y3 - y2)) ** 2)
            #print("distance t3=x3,x2: ",t3)
